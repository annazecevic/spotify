...
// Security event types for categorizing log events
const (
...
// LogEntry is a single structured JSON log record
...
// GetLogger returns the global logger (falls back to stdout if Init was not called)
...
// NewLogger creates a logger with file rotation (lumberjack), HMAC integrity, and sanitization
func NewLogger(cfg Config) *Logger {
	...
	// Log directory: 0700, log file: 0600
...
// log creates a sanitized, HMAC-signed JSON entry and writes it to all outputs
...
// Package-level functions (use global logger instance)
...
// Fields builds a details map from alternating key-value pairs:
// logger.Fields("user_id", "123", "ip", "1.2.3.4")
...
// computeHMAC generates HMAC-SHA256 over key entry fields for tamper detection
...
// sanitizeString masks emails and strips stack traces in production
