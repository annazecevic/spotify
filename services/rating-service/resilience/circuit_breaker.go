...
func (cb *CircuitBreaker) Allow() bool {
	...
	case StateOpen:
		if time.Since(cb.lastFailureTime) > cb.timeout {
			...
			cb.successCount = 0
			logger.Info(logger.EventGeneral, "Circuit breaker transitioning to HALF_OPEN", logger.Fields(
				"name", cb.name,
			))
			return true
		}
...
func (cb *CircuitBreaker) RecordSuccess() {
	...
	case StateHalfOpen:
		...
		if cb.successCount >= cb.successThreshold {
			...
			cb.failureCount = 0
			cb.successCount = 0
			logger.Info(logger.EventGeneral, "Circuit breaker CLOSED (recovered)", logger.Fields(
				"name", cb.name,
			))
		}
	case StateClosed:
...
func (cb *CircuitBreaker) RecordFailure() {
	...
	case StateClosed:
		...
		if cb.failureCount >= cb.failureThreshold {
			cb.state = StateOpen
			logger.Warn(logger.EventGeneral, "Circuit breaker OPEN (tripped)", logger.Fields(
				"name", cb.name,
	...
	case StateHalfOpen:
		cb.state = StateOpen
		cb.failureCount = 0
		logger.Warn(logger.EventGeneral, "Circuit breaker back to OPEN from HALF_OPEN", logger.Fields(
			"name", cb.name,
		))
	}
}
...
func Execute(
	...
	if !cb.Allow() {
		logger.Warn(logger.EventGeneral, "Circuit breaker is OPEN, using fallback", logger.Fields(
			"circuit", cb.name,
	...
	for attempt := 0; attempt <= retryCfg.MaxRetries; attempt++ {
		if attempt > 0 {
			...
			logger.Warn(logger.EventGeneral, "Retrying request", logger.Fields(
				"circuit", cb.name,
				"attempt", fmt.Sprintf("%d/%d", attempt, retryCfg.MaxRetries),
	...
	cb.RecordFailure()
	logger.Error(logger.EventGeneral, "All retries exhausted, using fallback", logger.Fields(
		"circuit", cb.name,
		"error", lastErr.Error(),
	))
	if fallback != nil {
